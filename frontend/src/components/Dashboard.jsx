...
export default function Dashboard() {
  ...
  const initialRois = {
    ...
    black: { x: 50, y: 350, w: 100, h: 100 },
    white: { x: 200, y: 350, w: 100, h: 100 },
  };
  ...
  const handleReset = () => {
    if (
      confirm(
        "Are you sure you want to RESET ALL? This will clear images, results, and locked Golden Sample data.",
      )
    ) {
  ...
  const HelpModal = () => (
      <div className="fixed inset-0 bg-black/80 z-[100] flex items-center justify-center p-4" onClick={() => setShowHelp(false)}>
          <div className="glass-panel max-w-2xl w-full p-8 relative text-left" onClick={e => e.stopPropagation()}>
              <button onClick={() => setShowHelp(false)} className="absolute top-4 right-4 text-white/50 hover:text-white">✕</button>
              <h2 className="text-2xl font-bold mb-6 text-cyan-400 text-center">使用說明 (Instructions)</h2>
              <div className="space-y-4 text-gray-300 text-sm overflow-y-auto max-h-[60vh] custom-scrollbar pr-2">
                  <section>
                      <h3 className="font-bold text-white mb-2">1. 準備工作</h3>
                      <p>請準備一張包含「白色區域」與「標準樣品」的影像。若需檢測透明度，背景需包含「黑色」與「白色」對照區域。</p>
                  </section>
                  <section>
                      <h3 className="font-bold text-white mb-2">2. 設定檢測區域 (ROI)</h3>
                      <ul className="list-disc pl-5 space-y-1">
                          <li><span className="text-yellow-400">校正白板 (Calibration)</span>：框選畫面中的純白區域，用於自動白平衡校正。</li>
                          <li><span className="text-cyan-400">標準樣品 (Golden Sample)</span>：框選合格的標準件，作為比對基準。可點擊「鎖定標準樣品」將其數值存檔。</li>
                          <li><span className="text-purple-400">測試樣品 (Test Product)</span>：框選待測產品，系統將計算其與標準樣品的色差 (Delta E)。</li>
                          <li><span className="text-gray-400">黑背景 (Black Bg)</span> & <span className="text-gray-200">白背景 (White Bg)</span>：(選用) 若需檢測透明度，請分別框選透過樣品看到的黑色與白色背景區域。</li>
                      </ul>
                  </section>
                  <section>
                      <h3 className="font-bold text-white mb-2">3. 執行檢測</h3>
                      <p>設定完成後，點擊 <span className="text-cyan-400 font-bold">🔍 執行檢測 (INSPECT)</span>。系統將自動計算色差與透明度，並判定是否合格 (Delta E &lt; 2.0)。</p>
                  </section>
                  <section>
                      <h3 className="font-bold text-white mb-2">操作技巧</h3>
                      <ul className="list-disc pl-5 space-y-1">
                          <li>使用 <span className="text-blue-400">🔍 區域縮放 (Area Zoom)</span> 來精確框選細小區域。</li>
                          <li>可隨時點擊 <span className="text-red-400">🗑️ 全部重置 (RESET ALL)</span> 清除所有設定。</li>
                      </ul>
                  </section>
              </div>
          </div>
  ...
  const captureFrame = () => {
    ...
    if (
      ...
      canvas.width = uploadedImgRef.current.width;
      canvas.height = uploadedImgRef.current.height;
    ...
    try {
      ...
      if (src.channels() === 4)
        window.cv.cvtColor(src, rgb, window.cv.COLOR_RGBA2RGB);
      else if (src.channels() === 1)
        window.cv.cvtColor(src, rgb, window.cv.COLOR_GRAY2RGB);
      else src.copyTo(rgb);
      src.delete();
      return rgb;
    } catch (e) {
  ...
  const lockStandard = () => {
    ...
    try {
      if (rois.ref.w <= 0 || rois.standard.w <= 0)
        throw new Error(
          "Please draw boxes for Color Card and Golden Sample first.",
        );
      let calibrated = ProcessingEngine.calibrateImage(mat, rois.ref);
      let resStd = ProcessingEngine.calculateDeltaE(
        calibrated,
        null,
        rois.standard,
      );
      ...
      setLockedStandard({ lab: resStd.lab, image: thumb });
          alert("成功：標準樣品數值已鎖定 (Success: Golden Sample values saved).");
          calibrated.delete();
    } catch (e) {
      alert(e.message);
    } finally {
  ...
  const analyze = () => {
    ...
    try {
      if (rois.ref.w <= 0)
        throw new Error("Please draw a box for the Color Card.");
      let calibrated = ProcessingEngine.calibrateImage(mat, rois.ref);
      let labStd;
      if (lockedStandard) {
        labStd = lockedStandard.lab;
      } else {
        if (rois.standard.w <= 0)
          throw new Error("Please draw the Golden Sample box.");
        let resStd = ProcessingEngine.calculateDeltaE(
          calibrated,
          null,
          rois.standard,
      ...
      if (rois.test.w <= 0)
        throw new Error("Please draw the Test Product box.");
          let resTest = ProcessingEngine.calculateDeltaE(calibrated, labStd, rois.test);
          
          let transparency = null;
          if (rois.black.w > 0 && rois.white.w > 0) {
              // Calculate transparency if ROIs are defined
              transparency = ProcessingEngine.checkTransparency(calibrated, rois.black, rois.white);
          }
          
          let transparencyDiff = Math.abs(labStd[0] - resTest.lab[0]); 
          setResults({ delta_e: resTest.deltaE, contrast_diff: transparencyDiff, transparency, lab_std: labStd, lab_test: resTest.lab, passed: resTest.deltaE < 2.0 });
          calibrated.delete();
    } catch (e) {
      console.error(e);
      alert(e.message);
  ...
  const DisplayArea = () => (
    <div
      className="relative border border-white/10 rounded-2xl bg-black shadow-2xl overflow-hidden"
    ...
    >
      ...
      <div className="absolute top-2 right-2 z-50 flex gap-2 pointer-events-none">
              <div className="glass-panel p-1 flex gap-1 rounded-lg pointer-events-auto">
                  <button onClick={() => setScale(s => Math.max(0.1, s * 0.8))} className="p-2 hover:bg-white/20 rounded" title="縮小 (Zoom Out)">➖</button>
                  <span className="p-2 text-xs font-bold w-16 text-center tabular-nums">{(scale * 100).toFixed(0)}%</span>
                  <button onClick={() => setScale(s => Math.min(10, s * 1.2))} className="p-2 hover:bg-white/20 rounded" title="放大 (Zoom In)">➕</button>
                  <button onClick={fitToScreen} className="p-2 hover:bg-white/20 rounded text-xs ml-2 px-3 border-l border-white/10">適應螢幕 (Fit)</button>
                  <button 
        ...
        <div className="min-w-full min-h-full grid place-items-center p-10">
          ...
          >
            {sourceType === "camera" && stream ? (
              ...
              />
            ) : sourceType === "upload" && uploadedImageSrc ? (
              <img
            ...
            {containerRef.current &&
              ...
              Object.entries(rois).map(([key, r]) => {
                ...
                return (
                  ...
                  >
                    <span
                      className={`bg-${color}/80 text-black px-1 absolute -top-5 left-0 text-[10px] font-bold whitespace-nowrap scale-item`}
                    >
                      {key.toUpperCase()}
                      {key === "standard" && lockedStandard ? " (LOCKED)" : ""}
                    </span>
                  </div>
                );
              })}
            ...
            {isDrawing && currentRect && (
              ...
              >
                {selectionMode === "zoom" && (
                  <span className="absolute -top-5 left-0 text-blue-400 text-xs font-bold bg-black/50 px-1">
                    ZOOM AREA
                  </span>
                )}
              </div>
          ...
          {selectionMode && !isDrawing && <div className="absolute top-16 left-1/2 -translate-x-1/2 bg-black/80 px-4 py-2 rounded-full text-white text-xs border border-white/20 animate-bounce pointer-events-none shadow-lg z-40 flex items-center gap-2">
              {selectionMode === 'zoom' ? '🔍 拖曳滑鼠框選放大區域' : <span>🖌️ 請框選區域: <span className="font-bold text-green-400 ml-2">{
                  {ref:'校正白板', standard:'標準樣品', test:'測試樣品', black:'黑背景', white:'白背景'}[selectionMode] || selectionMode.toUpperCase()
              }</span></span>}
          </div>}
    </div>
  );
        ...
        <div className="lg:col-span-2 space-y-6 flex flex-col h-full">
            ...
            <div className="flex justify-between items-center mb-2 px-1">
                <div className="flex items-center gap-4">
                    ...
                    <button onClick={() => setShowHelp(true)} className="px-3 py-1 bg-blue-500/10 hover:bg-blue-500/20 text-blue-400 hover:text-blue-300 rounded-full text-xs border border-blue-500/20 transition-all flex items-center gap-2">
                        ℹ️ 使用說明
                    </button>
                </div>
                <button onClick={handleReset} className="px-3 py-1 bg-red-500/10 hover:bg-red-500/20 text-red-400 hover:text-red-300 rounded text-xs border border-red-500/20 transition-all flex items-center gap-2 group">
                    <span className="group-hover:rotate-12 transition-transform">🗑️</span> 全部重置
                </button>
            </div>
          ...
          <DisplayArea />
            ...
            <div className="flex flex-wrap gap-4 justify-center">
                { !sourceType ? (
                    <>
                    <button onClick={() => fileInputRef.current.click()} className="btn-secondary flex items-center gap-2 text-lg px-6">📁 載入圖片 (Load Image)</button>
                    <button onClick={initCamera} className="btn-secondary">📷 啟動相機 (Camera)</button>
                    </>
                ) : (
                    <button onClick={() => fileInputRef.current.click()} className="btn-secondary flex items-center gap-2">📁 更換圖片</button>
                )}
            </div>

            {/* Step 2 & Controls */}
            <div className="glass-panel p-6">
                <h3 className="text-sm font-bold text-art-muted uppercase mb-4 text-center">步驟 2: 設定檢測區域 (Draw Regions)</h3>
                <div className="grid grid-cols-3 gap-3">
                    {[
                        { k: 'ref', label: '1. 校正白板', color: 'text-yellow-400', desc: 'White Balance' },
                        { k: 'standard', label: '2. 標準樣品', color: 'text-cyan-400', desc: 'Target Item' },
                        { k: 'test', label: '3. 測試樣品', color: 'text-purple-400', desc: 'Sample Check' },
                        { k: 'black', label: '4. 黑背景', color: 'text-gray-400', desc: 'Transparency Ref' },
                        { k: 'white', label: '5. 白背景', color: 'text-gray-200', desc: 'Transparency Ref' }
                    ].map(tool => (
                  ...
                  <div className={`text-sm font-bold ${tool.color}`}>
                    {tool.label}
                  </div>
                  <div className="text-[10px] text-art-muted mt-1">
                    {tool.desc}
                  </div>
                </button>
              ))}
          ...
          <div className="space-y-6">
            <div className="glass-panel p-8 text-center bg-gradient-to-b from-white/5 to-transparent min-h-[200px] flex flex-col justify-center">
                <h3 className="text-xl text-art-muted uppercase tracking-widest mb-4">檢測結果 (Result)</h3>
            ...
            <div className="grid grid-cols-1 gap-4">
                 {lockedStandard && (
                     <div className="glass-panel p-3 flex items-center gap-4 bg-cyan-900/20 border-cyan-500/30">
                         ...
                         <div>
                             <div className="text-xs text-cyan-400 font-bold uppercase mb-1">標準樣品 (Active)</div>
                             <div className="text-[10px] text-art-muted">使用已鎖定的數值</div>
                         </div>
                     </div>
                 )}
                 
                 <div className="metric-card">
                    <span className="text-art-muted text-sm uppercase">色差 (Delta E)</span>
                    ...
                    <div className="flex justify-between w-full text-[10px] text-art-muted mt-1"><span>目標 &lt; 2.0</span><span>當前值</span></div>
                 </div>
                 
                 <div className="metric-card">
                    <span className="text-art-muted text-sm uppercase">透明度 (Transparency)</span>
                    <span className="text-6xl font-black tracking-tight mt-2">{results && results.transparency !== null ? (results.transparency * 100).toFixed(0) + '%' : '--'}</span>
                    <div className="w-full bg-gray-700 h-2 rounded-full mt-4 overflow-hidden">
                        <div className="h-full bg-blue-400" style={{width: results && results.transparency !== null ? `${results.transparency * 100}%` : '0%'}}></div>
                    </div>
                    <div className="flex justify-between w-full text-[10px] text-art-muted mt-1"><span>0% (不透明)</span><span>100% (透明)</span></div>
                 </div>
                 
                 {results && (
                     <div className="glass-panel p-4 text-xs font-mono space-y-2">
                         <div className="flex justify-between"><span className="text-cyan-400">標準 Lab</span><span>{results.lab_std.map(v=>v.toFixed(0)).join(', ')}</span></div>
                         <div className="flex justify-between"><span className="text-purple-400">測試 Lab</span><span>{results.lab_test.map(v=>v.toFixed(0)).join(', ')}</span></div>
                     </div>
                 )}
            </div>
          </div>
